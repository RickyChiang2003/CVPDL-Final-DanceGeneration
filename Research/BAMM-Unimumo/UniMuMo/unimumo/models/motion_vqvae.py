...
class MotionVQVAE(pl.LightningModule):
    def __init__(
        ...
        pretrained_music_vqvae: tp.Optional[tp.Any] = None
    ):
        super().__init__()
        self.motion_key = motion_key
        self.music_key = music_key

        if pretrained_music_vqvae is not None:
            self.music_encoder = pretrained_music_vqvae.encoder
            self.quantizer = pretrained_music_vqvae.quantizer
            # Ensure frozen
            for p in self.music_encoder.parameters():
                p.requires_grad = False
            self.quantizer.freeze_codebook = True
        else:
