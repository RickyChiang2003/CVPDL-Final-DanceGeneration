...
class LMModel(StreamingModule):
    ...
    def get_self_attn_mask(self, section_1: int, section_2: int, mode: str) -> torch.Tensor:
        ...
        if mode in ['music_caption', 'motion_caption']:
            ...
            mask[section_1:, section_1:] = True
        elif mode == 'motion_inpaint':
            # section_1: Music, section_2: Motion
            # Base causal mask
            ...
            # Allow Middle to attend to Last
            # Assume section_2 is split 1:1:1 (8 beats, 8 beats, 8 beats)
            third = section_2 // 3
            mid_start = third
            mid_end = section_2 - third
            
            # Rows (Middle)
            row_start = section_1 + mid_start
            row_end = section_1 + mid_end
            
            # Cols (Last)
            col_start = section_1 + mid_end
            col_end = section_1 + section_2
    ...
    def generate(
        ...
        assert mode in ['music_motion', 'music2motion', 'motion2music', 'motion_inpaint']
        if mode != 'motion_inpaint':
        ...
        for offset in tqdm(range(1, gen_sequence_len), desc=f"Generating music & motion of shape {music_gen_sequence.shape}"):
            ...
            # i.e., update the prediction if they are not provided or are masked
