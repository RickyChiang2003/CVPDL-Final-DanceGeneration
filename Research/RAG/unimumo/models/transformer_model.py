...
class MusicMotionTransformer(pl.LightningModule):
    def __init__(
        ...
        assert stage is None or stage in ['train_music_motion', 'train_caption', 'train_motion_inpaint']
        self.stage = stage
        if self.stage == 'train_music_motion':
            print('In training music motion stage!')
        if self.stage == 'train_motion_inpaint':
            print('In training motion inpaint stage!')
    ...
    def get_pretrained_lm(
        ...
        assert stage in ['train_music_motion', 'train_caption', 'train_motion_inpaint']
    ...
    def setup_trainable_parameters(self):
        if self.stage in ['train_music_motion', 'train_motion_inpaint']:
    ...
    def training_step(
        ...
        if self.stage == 'train_music_motion':  # train the music motion lm
        ...
        elif self.stage == 'train_motion_inpaint':
            mode = 'motion_inpaint'
            text_condition = self.prepare_text_condition(text_cond)
            ...
            # Create inpaint mask for loss (Middle 33%)
            B, K, T = motion_code.shape
            third = T // 3
            if third > 0:
                # mask out First and Last thirds (keep Middle)
                inpaint_mask = torch.zeros_like(motion_mask)
                inpaint_mask[..., third:2*third] = 1
                motion_mask = motion_mask * inpaint_mask
            else:
            ...
            log_dict = {}
            for k in range(len(motion_loss_per_codebook)):
                log_dict[f'train/motion_ce_q{k + 1}'] = motion_loss_per_codebook[k]
    ...
    def validation_step(
        ...
        if self.stage == 'train_music_motion':
            ...
            quarter = T // 4
            inpaint_mask = torch.zeros_like(motion_mask)
            inpaint_mask[..., quarter:3*quarter] = 1
            motion_mask = motion_mask * inpaint_mask
    ...
    def compute_cross_entropy(
        ...
        for k in range(K):
            ...
            if ce_targets.numel() > 0:
                ...
                q_ce = torch.tensor(0.0, device=ce_targets.device, requires_grad=True)
            ce += q_ce
