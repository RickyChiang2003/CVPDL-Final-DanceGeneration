...
def plot_3d_motion(save_path, kinematic_tree, joints, title, vbeat, figsize=(10, 10), fps=120, radius=4, custom_color_func=None):
    ...
    def update(index):
        ...
        if custom_color_func is not None:
            custom_c = custom_color_func(index)
            if custom_c is not None:
                current_colors = custom_c

        for i, (chain, color) in enumerate(zip(kinematic_tree, current_colors)):
            if i < 5:
                linewidth = 4.0
            else:
                linewidth = 2.0
