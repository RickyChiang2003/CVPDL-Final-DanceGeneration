...
def visualize_music_motion(waveform: np.ndarray, joint: np.ndarray, save_dir: str, fps: int, filename: str = "video", custom_color_func=None):
    if waveform.ndim == 3:
    ...
    for i in range(batch_size):
        music_path = pjoin(save_dir, f'{filename}_{i}_music.mp3')
        try:
        ...
        motion_path = pjoin(save_dir, f'{filename}_{i}_motion.mp4')
        try:
            skel_animation.plot_3d_motion(
                motion_path, kinematic_chain, joint[i], title='Music-Motion', vbeat=None,
                fps=fps, radius=4, custom_color_func=custom_color_func
            )
        except Exception as e:
            print(e)
            continue

        video_path = pjoin(save_dir, f'{filename}_{i}.mp4')
        try:
            subprocess.call(
                f"ffmpeg -y -i {motion_path} -i {music_path} -c copy {video_path}",
                shell=True)
        except Exception as e:
            print(e)
            continue
